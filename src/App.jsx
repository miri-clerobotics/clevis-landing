    ...
    <div className="bg-[#F3F4F6] rounded-[6px] pt-3.5 pb-2 px-3.5 flex flex-col gap-2.5 overflow-hidden text-left">
      ...
      <div className="flex justify-between items-center mt-1">
        <div className="bg-white border border-[#E5E7EB] rounded-full px-2 py-0.5 flex items-center gap-1">
          ...
          <div className="text-[#4B5563] text-[9px] font-bold uppercase">LS electric</div>
        </div>
        <div className="flex items-center gap-1 text-[#9CA3AF] text-[10px] font-medium">0.142s <Clock size={12} /></div>
      </div>
    </div>
      ...
      {[1, 2].map((_, idx) => (
        <div key={idx} className="relative flex justify-between items-center gap-3 px-2">
          ...
          <div className="flex-1 bg-[#F9FAFB] border border-[#E5E7EB] rounded-[6px] py-1 px-2.5 flex flex-col items-end">
            <span className="text-[#9CA3AF] text-[8px] font-medium mb-0.5">Nullable</span>
...
export default function App() {
  ...
  // 스크롤 감지 최신 표준 (useMotionValueEvent)
  ...
  return (
    <div ref={containerRef} className="w-full bg-[#F8F9FB] relative flex flex-col font-sans selection:bg-red-500 selection:text-white pt-[140px]">
      ...
      {/* 0. UPDATED FLOATING HEADER (584px x 60px, Top 32px) */}
      <nav className="fixed top-8 left-1/2 -translate-x-1/2 w-[calc(100%-40px)] md:w-[584px] h-[60px] z-[100] px-6 py-2 flex items-center gap-x-6 bg-white/40 backdrop-blur-xl border border-white/20 rounded-full shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)] transition-all">
        ...
        <div className="flex items-center gap-6 text-[11px] font-bold text-gray-500 tracking-[0.05em]">
          {['Features', 'Library', 'Contact'].map(item => (
            <button key={item} onClick={() => scrollToSection(item.toLowerCase())} className="hover:text-[#0F172A] transition-colors uppercase whitespace-nowrap cursor-pointer">{item}</button>
          ))}
        </div>
      </nav>
      ...
      <section id="home" className="min-h-[70vh] relative flex flex-col items-center justify-center p-8 overflow-hidden">
        <div className="relative z-20 perspective-[2500px] mb-20">
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="mb-16 text-center md:text-left flex flex-col items-center md:items-start">
            <div className="inline-flex items-center py-2 px-4 border border-gray-200 bg-white rounded-full backdrop-blur-md shadow-inner mb-5">
              <span className="text-[12px] font-bold tracking-[0.05em] uppercase text-red-400">Core strengths</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight leading-tight text-[#0F172A]">산업 현장을 혁신하는 지능형 기술</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
            {strengths.map((s, i) => (
              <div key={i} className="p-10 rounded-3xl border border-gray-100 bg-[#F9FAFB] hover:bg-white hover:shadow-2xl transition-all group">
                <div className="w-12 h-12 rounded-2xl bg-red-50 flex items-center justify-center text-red-500 mb-8 transition-colors group-hover:bg-red-400 group-hover:text-white"><s.icon size={26} /></div>
          ...
          <div className="sticky top-0 h-screen w-full flex flex-col lg:flex-row items-center overflow-hidden z-20">
            <div className="w-full lg:w-1/2 h-full flex flex-col justify-center px-12 md:px-24 bg-white border-r border-gray-100">
                <div className="inline-flex w-fit items-center py-2 px-4 border border-gray-200 bg-white rounded-full backdrop-blur-md shadow-inner mb-5">
                    <span className="text-[12px] font-bold tracking-[0.05em] uppercase text-red-400">Continuing to update</span>
                </div>
                <h2 className="text-3xl md:text-4xl font-bold tracking-tight leading-tight text-[#0F172A]">Advanced Node Library</h2>
            ...
            <div className="w-full lg:w-1/2 h-full relative flex flex-col items-center justify-center p-12 bg-gray-50/50">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={activeIndex} initial={{ opacity: 0, scale: 0.9, x: 40 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.9, x: -40 }} transition={{ type: "spring", stiffness: 100, damping: 25 }} className="relative z-10">
                ...
                <AnimatePresence mode="wait">
                  <motion.div key={activeIndex} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.5 }}>
                    <div className="inline-flex items-center bg-red-400/10 border border-red-400/20 rounded-lg px-5 py-2 mb-5">
                      <span className="text-red-500 text-sm font-black tracking-[0.15em]">GROUP {activeIndex + 1}</span>
                    </div>
                    ...
                    <p className="text-gray-500 text-sm md:text-xl leading-relaxed font-medium max-w-lg">{nodeGroups[activeIndex].desc}</p>
      ...
      <section id="application" className="min-h-screen flex flex-col justify-center py-32 bg-white relative z-10 text-center px-8">
        <div className="container mx-auto max-w-6xl">
            <div className="mb-16 text-center md:text-left flex flex-col items-center md:items-start">
                <div className="inline-flex items-center py-2 px-4 border border-gray-200 bg-white rounded-full backdrop-blur-md shadow-inner mb-5">
                    <span className="text-[12px] font-bold tracking-[0.05em] uppercase text-red-400">Industrial applications</span>
                </div>
                <h2 className="text-3xl md:text-4xl font-bold tracking-tight leading-tight text-[#0F172A]">산업분야별 혁신적인 적용 사례</h2>
            </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-16">
            {cases.map((c, i) => (
              <div key={i} className="group cursor-pointer text-center">
                <div className="aspect-video rounded-3xl bg-[#F9FAFB] mb-6 overflow-hidden relative shadow-md border border-gray-100 transition-shadow hover:shadow-lg">
      ...
      {/* 5. CONTACT SECTION (KB 스타일 카드 - 호버 위치 고정) */}
      <section id="contact" className="min-h-[80vh] flex flex-col justify-center items-center py-24 bg-white relative overflow-hidden border-t border-gray-100 px-8">
        <div className="container mx-auto max-w-6xl text-center">
            <div className="mb-16 text-center md:text-left flex flex-col items-center md:items-start">
                <div className="inline-flex items-center py-2 px-4 border border-gray-200 bg-white rounded-full backdrop-blur-md shadow-inner mb-5">
                    <span className="text-[12px] font-bold tracking-[0.05em] uppercase text-red-400">Contact</span>
                </div>
                <h2 className="text-3xl md:text-4xl font-bold tracking-tight leading-tight text-[#0F172A]">클레비스 문의하기</h2>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-16 max-w-5xl mx-auto">
            ...
            <div 
              ...
              className="flex flex-col items-start p-10 bg-white rounded-[24px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.04)] transition-all hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:border-gray-300 group text-left h-full cursor-pointer relative"
            >
              <div className="relative w-[56px] h-[56px] mb-8 flex items-center justify-center">
                <div className="absolute inset-0 bg-red-50 rounded-[16px] group-hover:bg-red-400 transition-colors" />
                <MessageSquare className="text-red-400 transition-colors group-hover:text-white relative z-10" size={32} />
              </div>
              ...
              <div className="mt-auto flex items-center gap-2 text-[#6B7280] group-hover:text-red-500 font-bold text-[12px] uppercase tracking-tight transition-colors">
                <div className="w-[12px] h-[12px] flex items-center justify-center shrink-0">
                    <svg width="12" height="12" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M14.5 1.5H5.5C3.84315 1.5 2.5 2.84315 2.5 4.5V15.5C2.5 17.1569 3.84315 18.5 5.5 18.5H14.5C16.1569 18.5 17.5 17.1569 17.5 15.5V4.5C17.5 2.84315 16.1569 1.5 14.5 1.5ZM4 4.5C4 3.67157 4.67157 3 5.5 3H14.5C15.3284 3 16 3.67157 16 4.5V15.5C16 16.3284 15.3284 17 14.5 17H5.5C4.67157 17 4 16.3284 4 15.5V4.5Z" fill="currentColor" />
                    </svg>
                </div>
                상세 문의하기
              </div>
            ...
            <div 
              onClick={copyEmail}
              className="flex flex-col items-start p-10 bg-white rounded-[24px] border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.04)] transition-all hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:border-gray-200 group text-left h-full cursor-pointer relative"
            >
              <div className="relative w-[56px] h-[56px] mb-8 flex items-center justify-center">
                <div className="absolute inset-0 bg-gray-50 rounded-[16px] group-hover:bg-[#0F172A] transition-colors" />
                <Mail className="text-gray-400 transition-colors group-hover:text-white relative z-10" size={32} />
              </div>
      ...
      <AnimatePresence>
        {isToastVisible && (
          <motion.div 
            ...
            className="fixed bottom-12 left-1/2 z-[300] bg-[#0F172A] text-white px-8 py-4 rounded-full shadow-2xl flex items-center gap-3 border border-white/10 backdrop-blur-md"
          >
              ...
              {isSubmitted ? (
                ...
                <>
                  ...
                  <form onSubmit={handleInquirySubmit} className="space-y-4 text-left">
                    ...
                    <input type="tel" required placeholder="연락처" value={inquiryData.phone} onChange={(e) => setInquiryData({...inquiryData, phone: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:border-red-400 transition-all text-[13px] font-medium shadow-none" />
                    <input type="email" required placeholder="이메일 주소" value={inquiryData.email} onChange={(e) => setInquiryData({...inquiryData, email: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:border-red-400 transition-all text-[13px] font-medium shadow-none" />
                    ...
                    <button type="submit" disabled={!inquiryData.agreed} className={`w-full py-4 rounded-xl font-bold transition-all text-white ${inquiryData.agreed ? 'bg-[#1c1c1c] hover:bg-red-500 cursor-pointer' : 'bg-gray-300 cursor-not-allowed'}`}>문의하기</button>
                  </form>
                </>
              )}
            </motion.div>
      ...
      <footer className="bg-[#f9f9f9] border-t border-gray-200 py-20 px-10 md:px-20 font-sans text-gray-500 text-[14px] leading-relaxed relative z-10 text-left">
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="mb-14 text-[13px]">
            <div className="font-bold mb-4 text-gray-700 uppercase tracking-wide text-left">클레로보틱스 주식회사</div>
